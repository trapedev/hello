...
public class TsubuyakiController {
    ...
    @GetMapping("/search")
    String getTsubuyakiWithKeyword(@RequestParam("keyword") String keyword, Model model) {
        // サービスからキーワードを含むつぶやきを取得
        List<Tsubuyaki> list = ts.getTsubuyakiWithKeyword(keyword);
        model.addAttribute("tsubuyakiList", list); // モデル属性にリストをセット
